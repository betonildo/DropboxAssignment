# 1 "src/dropboxServer.c"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/usr/include/stdc-predef.h" 1 3 4
# 1 "<command-line>" 2
# 1 "src/dropboxServer.c"
# 1 "include/dropboxServer.h" 1




# 1 "include/dropboxUtil.h" 1
...
# 21 "include/dropboxUtil.h" 2
# 46 "include/dropboxUtil.h"
...
int main(int argc, char *argv[])
{
 ...
 signal(
# 21 "src/dropboxServer.c" 3 4
       2
# 21 "src/dropboxServer.c"
             , signal_callback_handler);
 signal(
# 22 "src/dropboxServer.c" 3 4
       11
# 22 "src/dropboxServer.c"
              , signal_callback_handler);
 signal(
# 23 "src/dropboxServer.c" 3 4
       3
# 23 "src/dropboxServer.c"
              , signal_callback_handler);
 ...
 if ((sockfd = socket(
# 35 "src/dropboxServer.c" 3 4
                     2
# 35 "src/dropboxServer.c"
                            , 
# 35 "src/dropboxServer.c" 3 4
                              SOCK_STREAM
# 35 "src/dropboxServer.c"
  ...
  exit(
# 37 "src/dropboxServer.c" 3 4
      (*__errno_location ())
# 37 "src/dropboxServer.c"
           );
 }
 ...
 int option = 2;
 setsockopt(sockfd, 
# 42 "src/dropboxServer.c" 3 4
                   1
# 42 "src/dropboxServer.c"
                             , 
# 42 "src/dropboxServer.c" 3 4
                               2 
# 42 "src/dropboxServer.c"
...
# 44 "src/dropboxServer.c" 3 4
                       2
# 44 "src/dropboxServer.c"
                              ;
 serv_addr.sin_port = htons(port);
 serv_addr.sin_addr.s_addr = 
# 46 "src/dropboxServer.c" 3 4
                            ((in_addr_t) 0x00000000)
# 46 "src/dropboxServer.c"
                                      ;
 ...
 if (bind(sockfd, (struct sockaddr *) &serv_addr, sizeof(serv_addr)) < 0) {
  ...
  exit(
# 51 "src/dropboxServer.c" 3 4
      (*__errno_location ())
# 51 "src/dropboxServer.c"
           );
 }
...
# 63 "src/dropboxServer.c" 3 4
                          ((void *)0)
# 63 "src/dropboxServer.c"
                              , receiveCommand, (void*)&newsockfd);
  }
 }
...
# 88 "src/dropboxServer.c" 3 4
         2 
# 88 "src/dropboxServer.c"
                == 
# 88 "src/dropboxServer.c" 3 4
                   (*__errno_location ())
# 88 "src/dropboxServer.c"
  ...
  int status = mkdir("users/", 0700);

  if (status < 0) {
   perror("mkdir() error");
   exit(
# 94 "src/dropboxServer.c" 3 4
       (*__errno_location ())
# 94 "src/dropboxServer.c"
            );
  }
 }
  ...
  exit(
# 100 "src/dropboxServer.c" 3 4
      (*__errno_location ())
# 100 "src/dropboxServer.c"
           );
 }
}
...
# 115 "src/dropboxServer.c" 3 4
                          ((void *)0)
# 115 "src/dropboxServer.c"
                              ;
 bool is_connected = false;


 { pthread_mutex_lock((&clients_mutex)); {{ clientnode = find_node_by_userid(clientslist, username); if (clientnode == 
# 119 "src/dropboxServer.c" 3 4
((void *)0)
# 119 "src/dropboxServer.c"
) { clientnode = make_node(); } if (clientnode->client == 
# 119 "src/dropboxServer.c" 3 4
((void *)0)
# 119 "src/dropboxServer.c"
) { clientnode->client = make_client(username); insert_client(clientslist, clientnode->client); printf("username: %s\n", clientnode->client->userid); } is_connected = try_connect_client_device(clientnode, newsockfd); attach_thread_to_clientnode(clientnode, pthread_self()); printf("username: %s\n", clientnode->client->userid); printf("socketfd: %d\n", get_right_socket_by_thread_index(clientslist, pthread_self())); };}; pthread_mutex_unlock((&clients_mutex)); }
# 136 "src/dropboxServer.c"
   ;
 ...
 if (!client_has_more_than_one_device_connected(clientnode)) {
  { pthread_mutex_lock((&clients_mutex)); {{ remove_client(clientslist, clientnode->client); };}; pthread_mutex_unlock((&clients_mutex)); }
...
int user_dir_exist(char* username) {
 ...
 char userhome[512] = "";
 sprintf(userhome, "%s%s/", "users/", username);
...
# 193 "src/dropboxServer.c" 3 4
         2 
# 193 "src/dropboxServer.c"
                == 
# 193 "src/dropboxServer.c" 3 4
                   (*__errno_location ())
# 193 "src/dropboxServer.c"
  ...
  perror("opendir() error");
  exit(
# 197 "src/dropboxServer.c" 3 4
      (*__errno_location ())
# 197 "src/dropboxServer.c"
           );
 }
...
void create_userhome_dir(char* username) {
 ...
 if (status < 0) {
  perror("mkdir() error");
  exit(
# 213 "src/dropboxServer.c" 3 4
      (*__errno_location ())
# 213 "src/dropboxServer.c"
           );
 }
}
...
void list_files() {
 ...
 for (i = 0; i < list.length; i++) {
  ...
  uint bytesWriten = write(socketfd, filePointer, sizeof(struct file_info));

 }

 struct file_info file;
 bzero(&file, sizeof(struct file_info));
 write(socketfd, &file, sizeof(struct file_info));
...
struct file_list getUsersFileList(char* username) {
 ...
 bzero(&list, sizeof(struct file_info));

 DIR* dir = opendir(userHome);


 if (dir == 
# 250 "src/dropboxServer.c" 3 4
           ((void *)0)
# 250 "src/dropboxServer.c"
               ) perror("opendir() error");
 else {
  struct dirent* entry = readdir(dir);
  for (;entry != 
# 253 "src/dropboxServer.c" 3 4
                ((void *)0)
# 253 "src/dropboxServer.c"
                    ; entry = readdir(dir)) {
   ...
   int isFile = fileStat.st_mode & 
# 272 "src/dropboxServer.c" 3 4
                                  0170000 
# 272 "src/dropboxServer.c"
                                         & 
# 272 "src/dropboxServer.c" 3 4
                                           0100000
# 272 "src/dropboxServer.c"
 ...
 if (dir != 
# 284 "src/dropboxServer.c" 3 4
           ((void *)0)
# 284 "src/dropboxServer.c"
               ) closedir(dir);
 return list;
}
...
struct file_info createFileInfoFromStats(struct dirent* entry, struct stat* fileStat) {
 ...
 strcat(file.last_modified, asctime(gmtime(&(*fileStat).
# 304 "src/dropboxServer.c" 3 4
                                                       st_mtim.tv_sec
# 304 "src/dropboxServer.c"
                                                               )));

 char* extStr = strchr(entry->d_name, '.');
 if (extStr != 
# 307 "src/dropboxServer.c" 3 4
              ((void *)0)
# 307 "src/dropboxServer.c"
                  ) strcat(file.extension, extStr + 1);
