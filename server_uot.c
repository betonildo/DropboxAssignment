...
int client_has_device_connected(struct client* client);
int try_connect_client_device(struct client* client, int device);
void disconnect_client_device(struct client* client, int device);
...
struct node* find_client_by_threadid(struct node* linkedlist, pthread_t threadid);
...
int main(int argc, char *argv[])
{
 ...
 if ((sockfd = socket(
# 24 "src/dropboxServer.c" 3 4
                     2
# 24 "src/dropboxServer.c"
                            , 
# 24 "src/dropboxServer.c" 3 4
                              SOCK_STREAM
# 24 "src/dropboxServer.c"
...
# 27 "src/dropboxServer.c" 3 4
                       2
# 27 "src/dropboxServer.c"
                              ;
 serv_addr.sin_port = htons(4000);
 serv_addr.sin_addr.s_addr = 
# 29 "src/dropboxServer.c" 3 4
                            ((in_addr_t) 0x00000000)
# 29 "src/dropboxServer.c"
                                      ;
...
# 59 "src/dropboxServer.c" 3 4
         2 
# 59 "src/dropboxServer.c"
                == 
# 59 "src/dropboxServer.c" 3 4
                   (*__errno_location ())
# 59 "src/dropboxServer.c"
  ...
  if (status < 0) {
   perror("mkdir() error");
   exit(
# 65 "src/dropboxServer.c" 3 4
       (*__errno_location ())
# 65 "src/dropboxServer.c"
            );
  }
 }
  ...
  exit(
# 71 "src/dropboxServer.c" 3 4
      (*__errno_location ())
# 71 "src/dropboxServer.c"
           );
 }
}
...
void* receiveCommand(void* clientSocket) {
 ...
 struct client* client = 
# 86 "src/dropboxServer.c" 3 4
                        ((void *)0)
# 86 "src/dropboxServer.c"
                            ;
 bool is_connected = false;


 { pthread_mutex_lock((&clients_mutex)); {({ client = find_client_by_userid(clientslist, username); if (!client) { client = make_client(username); insert_client(clientslist, client); } is_connected = try_connect_client_device(client, newsockfd); });}; } pthread_mutex_unlock((&clients_mutex));
 ...
 if (!client_has_device_connected(client)) {
  { pthread_mutex_lock((&clients_mutex)); {({ remove_client(clientslist, client); });}; } pthread_mutex_unlock((&clients_mutex));

    ;
 }
 else
  disconnect_client_device(client, newsockfd);
 close(newsockfd);
}
...
# 153 "src/dropboxServer.c" 3 4
         2 
# 153 "src/dropboxServer.c"
                == 
# 153 "src/dropboxServer.c" 3 4
                   (*__errno_location ())
# 153 "src/dropboxServer.c"
  ...
  perror("opendir() error");
  exit(
# 157 "src/dropboxServer.c" 3 4
      (*__errno_location ())
# 157 "src/dropboxServer.c"
           );
 }
...
void create_userhome_dir(char* username) {
 ...
 if (status < 0) {
  perror("mkdir() error");
  exit(
# 173 "src/dropboxServer.c" 3 4
      (*__errno_location ())
# 173 "src/dropboxServer.c"
           );
 }
}
...
void list_files() {
 ...
 struct file_list list = getUsersFileList(username);
 uint i;
 for (i = 0; i < list.length; i++) {
  void* filePointer = (void*)&list.files[i];
  uint bytesWriten = write(newsockfd, filePointer, sizeof(struct file_info));

 }

 struct file_info file;
 bzero(&file, sizeof(struct file_info));
 write(newsockfd, &file, sizeof(struct file_info));
...
struct file_list getUsersFileList(char* username) {
 ...
 if (dir == 
# 209 "src/dropboxServer.c" 3 4
           ((void *)0)
# 209 "src/dropboxServer.c"
               ) perror("opendir() error");
 else {
  struct dirent* entry = readdir(dir);
  for (;entry != 
# 212 "src/dropboxServer.c" 3 4
                ((void *)0)
# 212 "src/dropboxServer.c"
                    ; entry = readdir(dir)) {
...
# 231 "src/dropboxServer.c" 3 4
                                  0170000 
# 231 "src/dropboxServer.c"
                                         & 
# 231 "src/dropboxServer.c" 3 4
                                           0100000
# 231 "src/dropboxServer.c"
 ...
 if (dir != 
# 243 "src/dropboxServer.c" 3 4
           ((void *)0)
# 243 "src/dropboxServer.c"
               ) closedir(dir);
 return list;
}
...
# 263 "src/dropboxServer.c" 3 4
                                                       st_mtim.tv_sec
# 263 "src/dropboxServer.c"
                                                               )));

 char* extStr = strchr(entry->d_name, '.');
 if (extStr != 
# 266 "src/dropboxServer.c" 3 4
              ((void *)0)
# 266 "src/dropboxServer.c"
